...
export async function handlesToScan(
  ...
  const mains = take(
    rotated.filter(
      (h) => !extras.some((e) => e.toLowerCase() === h.toLowerCase()),
    ),
  ...
  if (catalog.length) {
    await beforeWrite?.();
    await cacheSetJson(
      CACHE_KEYS.scanCursor,
      (start + mains.length) % catalog.length,
      86_400,
    );
  }
...
export async function latestByAccount(
  beforeWrite?: () => Promise<void>,
  ...
  try {
    for (const section of listKnownSections()) {
      const res = await fetch(
        ...
        { headers: supabaseReadHeaders(), signal: AbortSignal.timeout(5_000) },
      );
      if (!res.ok) continue;
  ...
  if (out.size) {
    await beforeWrite?.();
    await cacheSetJson(CACHE_KEYS.newest, [...out.entries()], 60);
  }
  return out;
}
