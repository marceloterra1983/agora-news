...
export async function assetsForStory(story: Pick<Story, "id" | "source">): Promise<StoryAsset[]> {
  const embed = await embedForStory(story);
  return embed.assets;
}

export async function enrichStoryMedia(story: Story): Promise<Story> {
  const embed = await embedForStory(story);
  ...
  const poster = assets.find((a) => a.poster)?.poster;
  const kind = embed.quoted?.kind || embed.replyTo?.kind;
  return {
    ...story,
    assets,
    quoted: embed.quoted ?? story.quoted ?? null,
    replyTo: embed.replyTo ?? story.replyTo ?? null,
    card: embed.card ?? story.card ?? null,
    xArticle: embed.article ?? story.xArticle ?? null,
    image: photo?.url || poster || embed.quoted?.image || embed.article?.cover || story.image,
    media: assets.some((a) => a.type === "video")
      ? "Vídeo"
      : kind === "repost"
        ? "Repost"
        : kind === "quote"
