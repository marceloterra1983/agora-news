...
3. Make personalized `/api/feed` responses private/no-store and retain public-only behavior for anonymous requests → verify: `node --experimental-strip-types --test --test-name-pattern='cache|anonymous|owner' scripts/public-catalog-privacy.behavior.test.mjs scripts/release-gates.behavior.test.mjs && rg -q 'no new security findings in affected paths' specs/security/e04s08-audit-remediation.md`
